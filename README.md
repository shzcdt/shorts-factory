...
Удалённый пульт пайплайна. Создай бота у @BotFather, токен — в `.env` (`TELEGRAM_BOT_TOKEN`),
свой Telegram user id впиши в `config.yaml` → `telegram.allowed_users` (другим бот отвечает «нет доступа»).

```powershell
& .venv\Scripts\python.exe -m clip_pilot bot
```

| Команда | Что делает |
|---|---|
| `/status` | Счётчики источников/клипов по статусам, квоты, диск |
| `/sources` · `/clips <id>` | Списки источников и клипов |
| `/run scan\|analyze\|segment\|format` | Запуск стадии (subprocess) |
| `/pipeline` | Цепочка scan → analyze → segment → format |
| `/review` | Следующий клип на ревью: видео + кнопки ✅/❌/⏭ |
| `/approve_all` | Одобрить все клипы на ревью |
| `/publish [аккаунт] [лимит]` | Публикация одобренных клипов |
| `/accounts` | Аккаунты и наличие сессий |

Файлы, присланные боту: `.json` — сессия аккаунта (кладётся в `auth/`), видео ≤20 МБ — в `data/inbox`.
